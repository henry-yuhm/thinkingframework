...
public class SpecialOrderAllocator extends AbstractAllocator {
    ...
    public ShipmentCommand acquireCommand(ShipmentOrderDetail detail, Inventory inventory, BigDecimal quantity) throws Exception {
        ...
        if (command.getPickingList().getPicker() == null) {
            command.getPickingList().setPicker(this.getOperator());
            command.getPickingList().setPickingStartTime(Instant.now());
            command.getPickingList().setPickingCompleteTime(Instant.now());
        }
