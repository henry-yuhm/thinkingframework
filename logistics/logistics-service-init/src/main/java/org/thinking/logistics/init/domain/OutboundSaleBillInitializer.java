...
public class OutboundSaleBillInitializer extends AbstractBillInitializer {
    ...
    public OutboundSaleBillInitializer(Employee operator, OutboundHeader header) {
...
//            }
        ...
        if (address == null) {
            throw this.getException("未配置默认配送地址", this.header, this.header.getOwner(), this.header.getCustomer());
        }

        if (address.getDirection() == null) {
            throw this.getException("默认配送地址未配置配送方向", this.header, this.header.getOwner(), this.header.getCustomer());
        }
        ...
        if (this.isEnable(this.header.getWarehouse(), "ZTDDSFZDXF")) {
            if ((this.header.getTakegoodsMode() == TakegoodsMode.SELF_SERVICE || this.header.getTakegoodsMode() == TakegoodsMode.GREEN_CHANNEL) &&
                ...
                if (currentTime.after(trimTime)) {
                    throw this.getException("当前时间大于系统截单时间【" + trimTime.toString() + "】，不允许自动下发", this.header, this.header.getOwner());
                }
                //endregion
