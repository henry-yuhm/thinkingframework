...
public abstract class AbstractBillInitializer extends BusinessAdapter implements BillInitializer {
    ...
    public AbstractBillInitializer(Employee operator, OutboundHeader header) {
    ...
    public void verify() throws Exception {
        ...
        if (this.header.getStage().compareTo(OutboundStage.INITIALIZED) >= 0) {
            throw this.getException("单据已经初始化", this.header, this.header.getOwner());
        }

        if (this.header.getCustomer() == null) {
            throw this.getException(this.getNullCustomerMessage(), this.header, this.header.getOwner());
        }

        if (this.header.getDetails() == null || this.header.getDetails().isEmpty()) {
            throw this.getException(this.getNullDetailMessage(), this.header, this.header.getOwner());
        }

        for (OutboundDetail detail : this.header.getDetails()) {
            if (detail.getGoods() == null) {
                throw this.getException(this.getNullGoodsMessage(), this.header, this.header.getOwner());
            } else {
        ...
        if (message.length() > 0) {
            ...
            throw this.getException(message.toString(), this.header, this.header.getOwner());
        }
    }
